...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            elif "InternVL3" in model_id:
                # Handle InternVL3 models
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                # Handle InternVL3 reference model
                ref_model_init_kwargs = model_init_kwargs.copy()
                ref_model_init_kwargs.pop("use_cache", None)
                self.ref_model = AutoModelForCausalLM.from_pretrained(model_id, **ref_model_init_kwargs)
            else:
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Aria" in model_id or "Qwen2.5-VL" in model_id or "InternVL3" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Get model_id for model-specific handling
        model_id = model.config._name_or_path if hasattr(model, 'config') else "unknown"
        ...
        #         # Remove the 'text' key if it exists and add a 'video' key with the video path.
        #         new_inp['prompt'][0]['content'][0].pop("text", None)
        #         new_inp['prompt'][0]['content'][0]["video"] = inp["video"]
        #         _, video_info = process_vision_info(new_inp["prompt"])
        #         video_inputs.append(video_info)

        # # print("inputs", inputs)
        # # images, video_inputs, video_kwargs = process_vision_info(inputs, return_video_kwargs=True)
        ...
        if "image" in inputs[0]:
            ...
            # video = [x["video"] for x in inputs]
            # Build a list of conversation messages expected by process_vision_info.
            messages = []
            video_inputs = []
            for inp in inputs:
                # Start with the video element.
                print("input video", inp["video"])
                content = [{"type": "video", "video": inp["video"]}]
                # Append any text content from the prompt.
                for p in inp.get("prompt", []):
                    for item in p.get("content", []):
                        if item.get("type") == "text" and item.get("text") is not None:
                            content.append({"type": "text", "text": item["text"]})
                messages.append({"role": inp.get("role", "user"), "content": content})
            
                # Process the messages to extract video features.
                try:
                    images, video_input, video_kwargs = process_vision_info(messages, return_video_kwargs=True)
        ...
        # Debug: Print prompt_inputs keys for InternVL3
        if "InternVL3" in model_id:
            print(f"InternVL3 prompt_inputs keys: {list(prompt_inputs.keys())}")
        ...
        # print("len(prompt_completion_ids)", len(prompt_completion_ids))

        # # Okay I am assuming that the inputs are Qwen2VL processor
        # # and no video for now, repeat the image for each completion
        if "image" in inputs[0]:
            # Handle different model types for image inputs
            if "InternVL3" in model_id:
                # InternVL3 specific image handling
                ...
                # InternVL3 might have additional image-specific keys
                for key in list(prompt_inputs.keys()):
                    if key.startswith("image_") and isinstance(prompt_inputs[key], torch.Tensor):
                        if prompt_inputs[key].dim() > 0:
                            prompt_inputs[key] = prompt_inputs[key].repeat(len(prompt_completion_ids), 1)
                        else:
                            prompt_inputs[key] = prompt_inputs[key] * len(prompt_completion_ids)
            else:
                # Qwen2.5-VL image handling
                prompt_inputs["pixel_values"] = prompt_inputs["pixel_values"].repeat(len(prompt_completion_ids), 1)
                prompt_inputs["image_grid_thw"] = prompt_inputs["image_grid_thw"].repeat(len(prompt_completion_ids), 1)
        # import pdb; pdb.set_trace()


        # # XXX if input video --> qwen2.5 video processor
        if "video" in inputs[0]:
            # print("video input:", video_inputs[0].shape)
            # Repeat the video inputs so there is one per prompt completion.
            num_completions = len(prompt_completion_ids)
            
            # Handle different model types for video inputs
            if "InternVL3" in model_id:
                # InternVL3 specific video handling
                if "pixel_values_videos" in prompt_inputs:
                    prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat(num_completions, 1)
                if "video_grid_thw" in prompt_inputs:
                    prompt_inputs["video_grid_thw"] = prompt_inputs["video_grid_thw"].repeat(num_completions, 1)
                if "second_per_grid_ts" in prompt_inputs:
                    prompt_inputs["second_per_grid_ts"] = prompt_inputs["second_per_grid_ts"] * num_completions
                # InternVL3 might have additional video-specific keys
                for key in list(prompt_inputs.keys()):
                    if key.startswith("video_") and isinstance(prompt_inputs[key], torch.Tensor):
                        if prompt_inputs[key].dim() > 0:
                            prompt_inputs[key] = prompt_inputs[key].repeat(num_completions, 1)
                        else:
                            prompt_inputs[key] = prompt_inputs[key] * num_completions
            else:
                # Qwen2.5-VL video handling
                prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat(num_completions, 1)
                # print("prompt_inputs[pixel_values_videos].shape", prompt_inputs["pixel_values_videos"].shape)
                prompt_inputs["video_grid_thw"] = prompt_inputs["video_grid_thw"].repeat(num_completions, 1)
                # prompt_inputs["second_per_grid_ts"] = prompt_inputs["second_per_grid_ts"].repeat(num_completions, 1)

                prompt_inputs["second_per_grid_ts"] = prompt_inputs["second_per_grid_ts"] * num_completions


            # print("prompt_inputs[video_grid_thw].shape", prompt_inputs["video_grid_thw"].shape)
        ...
        try:
            ...
            print(f"Error in main model forward pass: {e}")
        ...
        with torch.inference_mode():
            if self.ref_model is not None:
                try:
                    ...
                    print(f"Error in ref_model forward pass: {e}")
                    print(f"prompt_completion_ids shape: {prompt_completion_ids.shape}")
                    print(f"prompt_inputs keys: {list(prompt_inputs.keys())}")
                    raise e
            else:
                with self.accelerator.unwrap_model(model).disable_adapter():
                    try:
                        ...
                        print(f"Error in model forward pass with adapter disabled: {e}")
                        print(f"prompt_completion_ids shape: {prompt_completion_ids.shape}")
                        print(f"prompt_inputs keys: {list(prompt_inputs.keys())}")
                        raise e
